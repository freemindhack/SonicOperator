...
class Cache {
	...
	int get_data(int index, float[][] data, int len)
	{
		...
		if(more > 0)
		{
			...
			h.data = new_data;
			size -= more;
			do {int temp_value= h.len; h.len=len; len=temp_value;} while(false);
		}
	...
	void swap_index(int i, int j)
	{
		...
		do {float[] temp_value=head[i].data; head[i].data=head[j].data; head[j].data=temp_value;} while(false);
		do {int temp_value=head[i].len; head[i].len=head[j].len; head[j].len=temp_value;} while(false);
		if(head[i].len > 0) lru_insert(head[i]);
		if(head[j].len > 0) lru_insert(head[j]);

		if(i>j) do {int temp_value=i; i=j; j=temp_value;} while(false);
		for(head_t h = lru_head.next; h!=lru_head; h=h.next)
		{
			if(h.len > i)
			{
				if(h.len > j)
					do {float temp_value=h.data[i]; h.data[i]=h.data[j]; h.data[j]=temp_value;} while(false);
				else
				{
					// give up
					lru_delete(h);
...
abstract class Kernel extends QMatrix {
	...
	void swap_index(int i, int j)
	{
		do {svm_node[] temp_value=x[i]; x[i]=x[j]; x[j]=temp_value;} while(false);
		if(x_square != null) do {double temp_value=x_square[i]; x_square[i]=x_square[j]; x_square[j]=temp_value;} while(false);
	}
...
class Solver {
	...
	{
		Q.swap_index(i,j);
		do {byte temp_value=y[i]; y[i]=y[j]; y[j]=temp_value;} while(false);
		do {double temp_value=G[i]; G[i]=G[j]; G[j]=temp_value;} while(false);
		do {byte temp_value=alpha_status[i]; alpha_status[i]=alpha_status[j]; alpha_status[j]=temp_value;} while(false);
		do {double temp_value=alpha[i]; alpha[i]=alpha[j]; alpha[j]=temp_value;} while(false);
		do {double temp_value=p[i]; p[i]=p[j]; p[j]=temp_value;} while(false);
		do {int temp_value=active_set[i]; active_set[i]=active_set[j]; active_set[j]=temp_value;} while(false);
		do {double temp_value=G_bar[i]; G_bar[i]=G_bar[j]; G_bar[j]=temp_value;} while(false);
	}
...
class SVC_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		...
		do {byte temp_value=y[i]; y[i]=y[j]; y[j]=temp_value;} while(false);
		do {double temp_value=QD[i]; QD[i]=QD[j]; QD[j]=temp_value;} while(false);
	}
}
...
class SVR_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		do {byte temp_value=sign[i]; sign[i]=sign[j]; sign[j]=temp_value;} while(false);
		do {int temp_value=index[i]; index[i]=index[j]; index[j]=temp_value;} while(false);
		do {double temp_value=QD[i]; QD[i]=QD[j]; QD[j]=temp_value;} while(false);
	}
...
public class svm {
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for(i=0;i<prob.l;i++)
		{
			...
			do {int temp_value=perm[i]; perm[i]=perm[j]; perm[j]=temp_value;} while(false);
		}
		for(i=0;i<nr_fold;i++)
		{
	...
	private static void svm_group_classes(svm_problem prob, int[] nr_class_ret, int[][] label_ret, int[][] start_ret, int[][] count_ret, int[] perm)
	{
		...
		{
			do {int temp_value=label[0]; label[0]=label[1]; label[1]=temp_value;} while(false);
			do {int temp_value=count[0]; count[0]=count[1]; count[1]=temp_value;} while(false);
			for(i=0;i<l;i++)
			{
				if(data_label[i] == 0)
					data_label[i] = 1;
		    ...
		    param.svm_type == svm_parameter.NU_SVC) && nr_fold < l)
		{
			...
			for (c=0; c<nr_class; c++)
				for(i=0;i<count[c];i++)
				{
					...
					do {int temp_value=index[start[c]+j]; index[start[c]+j]=index[start[c]+i]; index[start[c]+i]=temp_value;} while(false);
				}
			for(i=0;i<nr_fold;i++)
			{
				fold_count[i] = 0;
